(
	...
	function(){
		...
		tinymce.create(
			"tinymce.plugins.templatationShortcodes",
			{
				init: function(d,e) {
						var nonce = '';
						if ( nonce == '' ) {
						...
						d.addCommand( "wooVisittemplatation", function(){ window.open( "http://templatation.com/" ) } );
						...
						d.addCommand( "wooOpenDialog",function(a,c){
							...
							jQuery.get(e+"/dialog.php?woo-shortcodes-nonce=" + nonce,function(b){
								...
								tb_show( "Insert templatation "+ wooSelectedShortcodeTitle +" Shortcode", "#TB_inline?width="+f+"&height="+b+"&inlineId=woo-dialog" );jQuery( "#woo-options h3:first").text( "Customize the "+c.title+" Shortcode" );
						...
						// d.onNodeChange.add(function(a,c){ c.setDisabled( "templatation_shortcodes_button",a.selection.getContent().length>0 ) } ) // Disables the button if text is highlighted in the editor.
					},
					
				createControl:function(d,e){
				
						if(d=="templatation_shortcodes_button"){
						
							d=e.createMenuButton( "templatation_shortcodes_button",{
								title:"Insert templatation Shortcode",
								image:icon_url,
								icons:false
								});
		...
		/*b.add({title:"Visit templatation.com","class":"woo-woolink",onclick:function(){tinyMCE.activeEditor.execCommand( "wooVisittemplatation",false,"")}})*/ });
							return d
				...
				getInfo:function(){ return{longname:"templatation Shortcode Generator",author:"VisualShortcodes.com",authorurl:"http://visualshortcodes.com",infourl:"http://visualshortcodes.com/shortcode-ninja",version:"1.0"} }
			}
		);
		
		tinymce.PluginManager.add( "templatationShortcodes",tinymce.plugins.templatationShortcodes)
	}
)();
