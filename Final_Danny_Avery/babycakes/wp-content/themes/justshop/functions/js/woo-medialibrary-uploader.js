/*-----------------------------------------------------------------------------------*/
/* templatation Media Library-driven AJAX File Uploader Module - JavaScript Functions */
/* 2010-11-05. */
/*
/* The code below is designed to work as a part of the templatation Media Library-driven
...
/*-----------------------------------------------------------------------------------*/
	...
	$( 'input.upload_button' ).removeAttr( 'style' );
      ...
      $( 'input.upload_button').live( "click", function () {
       ...
       if ( woo_title == '' && $(this).parents( '.woo_metabox_fields' ).prev( 'th' ).find( 'label' ) ) { woo_title = $(this).parents( '.woo_metabox_fields' ).prev( 'th' ).find( 'label' ).text(); } // End IF Statement
        ...
        tb_show( woo_title, 'media-upload.php?post_id='+formID+'&amp;title=' + woo_title + '&amp;is_templatation=yes&amp;TB_iframe=1' );
        return false;
      });
  ...
  }; // End templatationMLU Object // Don't remove this, or the sky will fall on your head.

/*-----------------------------------------------------------------------------------*/
/* Execute the above methods in the templatationMLU object.
/*-----------------------------------------------------------------------------------*/
  
	$(document).ready(function () {

		templatationMLU.removeFile();
		templatationMLU.recreateFileField();
		templatationMLU.mediaUpload();
