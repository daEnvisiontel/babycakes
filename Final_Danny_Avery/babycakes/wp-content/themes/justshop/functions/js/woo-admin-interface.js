/**
 * templatation Admin Interface JavaScript
 *
		...
		$( '.group .collapsed' ).each(function(){
			$( this ).find( 'input:checked' ).parent().parent().parent().nextAll().each( function(){
				...
				$( '.group .collapsed input:checkbox').click(function ( e ) {
					templatationAdminInterface.unhide_hidden( $( this ).attr( 'id' ) );
				});
 ...
 */
  ...
  }; // End templatationAdminInterface Object // Don't remove this, or the sky will fall on your head.

/**
 * Execute the above methods in the templatationAdminInterface object.
 *
 * @since 4.8.0
 */
	$(document).ready(function () {
	
		templatationAdminInterface.toggle_nav_tabs();
		templatationAdminInterface.load_first_tab();
		templatationAdminInterface.toggle_collapsed_fields();
		templatationAdminInterface.setup_nav_highlights();
		templatationAdminInterface.toggle_nav_menus();
		templatationAdminInterface.init_flyout_menus();
		templatationAdminInterface.open_first_menu();
		templatationAdminInterface.banner_advert_close();
		templatationAdminInterface.setup_custom_typography();
		templatationAdminInterface.setup_custom_ui_slider();
