...
<?php
/*-----------------------------------------------------------------------------------*/
/* templatation Media Library-driven AJAX File Uploader Module */
/* 2010-11-05. */
/*
/* If we're on a templatation specific administration page, add Media Library Uploader
/* specific actions for CSS, JavaScript and several other functionalities.
/*-----------------------------------------------------------------------------------*/

if ( is_admin() ) {
	add_action( 'init', 'templatation_mlu_init' );
	add_action( 'admin_print_scripts', 'templatation_mlu_insidepopup' );
	add_filter( 'gettext', 'templatation_mlu_change_button_text', null, 2 );
	...
	if ( ( isset( $_REQUEST['page'] ) ) ) {
		...
		if ( ( $_page != '' && substr( $_page, 0, 12 ) == 'templatation' ) || $is_posts_page ) {
			add_action( 'admin_print_styles', 'templatation_mlu_css', 0 );
			add_action( 'admin_print_scripts', 'templatation_mlu_js', 0 );
		}
}

/*-----------------------------------------------------------------------------------*/
/* templatation_mlu_init */
/*
/* Global init() function for the templatation Media Library-driven AJAX File Uploader.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_mlu_init' ) ) {
	function templatation_mlu_init () {
		register_post_type( 'wooframework', array(
			'labels' => array(
				'name' => __( 'WooFramework Internal Container', 'templatation' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'templatation_mlu_js' ) ) {
	function templatation_mlu_js () {
	...
	} // End templatation_mlu_js()
}

/*-----------------------------------------------------------------------------------*/
/* templatation_medialibrary_uploader */
/*
/* templatation Uploader Using the WordPress Media Library.
/*
/* Parameters:
...
/*
/* Dependencies:
/* - templatation_mlu_get_silentpost()
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_medialibrary_uploader' ) ) {
	function templatation_medialibrary_uploader ( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0 ) {
		$output = '';
		...
		if ( $_postid != 0 ) {
			...
			$int = templatation_mlu_get_silentpost( $id ); // Change for each field, using a "silent" post. If no post is present, one will be created.
		}
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'templatation' ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'templatation' );
		...
		return $output;
	} // End templatation_medialibrary_uploader()
}
...
/*
/* Example Usage:
/* templatation_mlu_get_silentpost ( 'woo_logo' );
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_mlu_get_silentpost' ) ) {
	function templatation_mlu_get_silentpost ( $_token ) {
		global $wpdb;
		...
		return $_id;
	} // End templatation_mlu_get_silentpost()
}

/*-----------------------------------------------------------------------------------*/
/* templatation_mlu_insidepopup */
/*
/* Trigger code inside the Media Library popup.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_mlu_insidepopup' ) ) {
	function templatation_mlu_insidepopup () {
		if ( isset( $_REQUEST['is_templatation'] ) && $_REQUEST['is_templatation'] == 'yes' ) {
			add_action( 'admin_head', 'templatation_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'templatation_mlu_modify_tabs' );
		}
	} // End templatation_mlu_insidepopup()
}

if ( ! function_exists( 'templatation_mlu_js_popup' ) ) {
	function templatation_mlu_js_popup () {
		$_woo_title = 'file';
...
?>
	...
	jQuery(function($) {
		...
		// Preserve the "is_templatation" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_templatation=yes';
...
<?php
	...
	} // End templatation_mlu_js_popup()
}

/*-----------------------------------------------------------------------------------*/
/* templatation_mlu_modify_tabs */
/*
/* Triggered inside the Media Library popup to modify the title of the "Gallery" tab.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_mlu_modify_tabs' ) ) {
	function templatation_mlu_modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( __( 'Gallery', 'templatation' ), __( 'Previously Uploaded', 'templatation' ), $tabs['gallery'] ); }
		return $tabs;
	} // End templatation_mlu_modify_tabs()
} // End IF Statement

/*-----------------------------------------------------------------------------------*/
/* templatation_mlu_change_button_text */
/*
/* Change the "Insert Into Post" button text where appropriate.
/*-----------------------------------------------------------------------------------*/

if ( ! function_exists( 'templatation_mlu_change_button_text' ) ) {
	function templatation_mlu_change_button_text( $translation, $original ) {
	    ...
	    if( $original == 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'templatation' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation = sprintf( __( 'Use as %s', 'templatation' ), esc_attr( $_REQUEST['title'] ) ); }
	    }
	
	    return $translation;
	} // End templatation_mlu_change_button_text()
}
?>