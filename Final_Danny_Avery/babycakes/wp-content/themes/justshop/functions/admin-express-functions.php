...
function express_uploadFile($args) {
	...
	if ( !current_user_can('upload_files') ) {
		...
		return new IXR_Error(401, __('You are not allowed to upload files to this site.', 'templatation'));
	}
	...
	if ( ! empty($upload['error']) ) {
		$errorString = sprintf(__('Could not write file %1$s (%2$s)', 'templatation'), $name, $upload['error']);
...
function express_woo_taxonomy($args) {
	...
	// Re-assign the taxonomies so they are compatible with templatation themes
