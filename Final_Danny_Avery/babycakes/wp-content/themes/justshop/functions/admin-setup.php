...
define( 'THEME_FRAMEWORK', 'templatation' );
...
function woo_themeoptions_redirect () {
	header( 'Location: ' . admin_url( 'admin.php?page=templatation&activated=true' ) );
...
if ( ! function_exists( 'templatation_admin_head' ) ) {
/**
 * Optionally add markup in the header of the WordPress admin.
 * @since  4.0.0
 * @return void
 */
function templatation_admin_head() {} // End templatation_admin_head()
}
add_action( 'admin_head', 'templatation_admin_head', 10 );
...
function templatation_wp_head() {
	do_action( 'templatation_wp_head_before' );
	// Output alternative stylesheet
	...
	do_action( 'templatation_wp_head_after' );
} // End templatation_wp_head()
}
add_action( 'wp_head', 'templatation_wp_head', 10 );
...
function woo_output_alt_stylesheet() {
	...
	if ( $style != '' ) {
		...
		if( $style != '' ) {
			...
			if( $settings['disable_responsive'] == 'true' ) {
				echo '<link href="'. esc_url( get_template_directory_uri() . '/styles/non-responsive/'. $style ) . '" rel="stylesheet" type="text/css" />' . "\n";
			}
		} else {
...
function woo_output_custom_favicon () {
	...
	if( '' != get_option( 'woo_fvcn_57x57', '' ) ): ?>
	<!-- For iPhone -->
	<link rel="apple-touch-icon-precomposed" href="<?php echo get_option( 'woo_fvcn_57x57', '' ); ?>">
	<?php endif;

	if( '' != get_option( 'woo_fvcn_114x114', '' ) ): ?>
	<!-- For iPhone 4 Retina display -->
	<link rel="apple-touch-icon-precomposed" sizes="114x114" href="<?php echo get_option( 'woo_fvcn_114x114', '' ); ?>">
	<?php endif;

	if( '' != get_option( 'woo_fvcn_72x72', '' ) ): ?>
	<!-- For iPad -->
	<link rel="apple-touch-icon-precomposed" sizes="72x72" href="<?php echo get_option( 'woo_fvcn_72x72', '' ); ?>">
	<?php endif;

	if( '' != get_option( 'woo_fvcn_144x144', '' ) ): ?>
	<!-- For iPad Retina display -->
	<link rel="apple-touch-icon-precomposed" sizes="144x144" href="<?php echo get_option( 'woo_fvcn_144x144', '' ); ?>">
	<?php endif; 
...
function woo_load_textdomain () {
	load_theme_textdomain( 'templatation' );
	load_theme_textdomain( 'templatation', get_template_directory() . '/lang' );
	if ( function_exists( 'load_child_theme_textdomain' ) )
		load_child_theme_textdomain( 'templatation' );
...
function woo_output_custom_css() {
	$theme_dir = get_template_directory();	$theme_dir_uri = $custfile = "";
	if ( is_child_theme() ) { $theme_dir = get_stylesheet_directory(); }
	$custfile = $theme_dir . '/custom.css';
	if ( ! file_exists( $custfile ) ) { return; } // return if custom.css doesnt exists.
	
	$theme_dir_uri = get_template_directory_uri();	
	if ( is_child_theme() && file_exists( get_stylesheet_directory() . '/custom.css' ) )
	$theme_dir_uri = get_stylesheet_directory_uri();

	echo "\n" . '<!-- Custom Stylesheet -->' . "\n" . '<link href="'. esc_url( $theme_dir_uri . '/custom.css' ) . '" rel="stylesheet" type="text/css" />' . "\n";
...
function woo_comment_reply() {
	if ( is_singular() ) wp_enqueue_script( 'comment-reply','',false,true ); //@justshop1.0 making it load on bottom.
