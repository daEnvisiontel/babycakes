 ...
 * TABLE OF CONTENTS
 *
 * - templatation_metabox_create()
 * - templatation_metabox_create_fields()
 * - templatation_metabox_handle()
 * - templatation_metabox_add()
 * - templatation_metabox_fieldtypes()
 * - templatation_uploader_custom_fields()
 * - woo_custom_enqueue()
 ...
 * @return void
 */
function templatation_metabox_create( $post, $callback ) {
    global $post;

	// Allow child themes/plugins to act here.
	do_action( 'templatation_metabox_create', $post, $callback );
    ...
    if ( $callback['id'] == 'templatation-settings' ) {
	    // Add tabs.
	    $output .= '<div class="wooframework-tabs">' . "\n";
	    
	    $output .= '<ul class="tabber hide-if-no-js">' . "\n";
	    	if ( $display_general_fields ) {
                $output .= '<li class="wf-tab-general"><a href="#wf-tab-general">' . __( 'General Settings', 'templatation' ) . '</a></li>' . "\n";
            }
    ...
    if ( $display_general_fields ) {
        $output .= templatation_metabox_create_fields( $woo_metaboxes, $callback, 'general' );
    ...
    echo $output;
} // End templatation_metabox_create()
 ...
 */
function templatation_metabox_create_fields ( $metaboxes, $callback, $token = 'general' ) {
	global $post;
    ...
    foreach ( $metaboxes as $k => $woo_metabox ) {
    	...
    	$woo_id = 'templatation_' . $woo_metabox['name'];
    	...
    	if ( $type_selector ) {
    		...
    		if( isset( $woo_metabox['type'] ) && ( in_array( $woo_metabox['type'], templatation_metabox_fieldtypes() ) ) ) {
        	    ...
        	    elseif( $woo_metabox['type'] == 'upload' )
        	    {
        	    	...
        	    	// Add support for the templatation Media Library-driven Uploader Module // 2010-11-09.
        	    	if ( function_exists( 'templatation_medialibrary_uploader' ) ) {
	    	            ...
	    	            $output .= "\t\t".'<td class="woo_metabox_fields">'. templatation_medialibrary_uploader( $woo_metabox['name'], $_value, 'postmeta', $woo_metabox['desc'], $post->ID );
	    	            $output .= '</td>'."\n";
	    	            $output .= "\t".'</tr>'."\n";
	    	            ...
	    	            $output .= "\t".'<tr class="' . $row_css_class . '">';
	    	            $output .= "\t\t".'<th class="woo_metabox_names"><label for="'.esc_attr( $woo_id ).'">'.$woo_metabox['label'].'</label></th>'."\n";
	    	            $output .= "\t\t".'<td class="woo_metabox_fields">'. templatation_uploader_custom_fields( $post->ID, $woo_name, $default, $woo_metabox['desc'] );
	    	            $output .= '</td>'."\n";
	    	            $output .= "\t".'</tr>'."\n";
    ...
    return $output;
} // End templatation_metabox_create_fields()
 ...
 * @return void
 */
function templatation_metabox_handle( $post_id ) {
    $pID = '';
    global $globals, $post;
...
} // End templatation_metabox_handle()
 ...
 * @return void
 */
function templatation_metabox_add () {
	$woo_metaboxes = get_option( 'woo_custom_template', array() );
    if ( function_exists( 'add_meta_box' ) ) {
    	if ( function_exists( 'get_post_types' ) ) {
			...
			foreach ( $custom_post_list as $type ) {
				...
				$settings = array(
									'id' => 'templatation-settings',
									'title' => sprintf( __( '%s Custom Settings', 'templatation' ), $theme_name ), 
									'callback' => 'templatation_metabox_create',
									'page' => $type,
									'priority' => 'normal',
									'callback_args' => ''
								);
				...
				$settings = apply_filters( 'templatation_metabox_settings', $settings, $type, $settings['id'] );
    		...
    		add_meta_box( 'templatation-settings', sprintf( __( '%s Custom Settings', 'templatation' ), $theme_name ), 'templatation_metabox_create', 'post', 'normal' );
        	add_meta_box( 'templatation-settings', sprintf( __( '%s Custom Settings', 'templatation' ), $theme_name ), 'templatation_metabox_create', 'page', 'normal' );
    	}
    }
} // End templatation_metabox_add()
 ...
 * @return void
 */
function templatation_metabox_fieldtypes() {
	return apply_filters( 'templatation_metabox_fieldtypes', array( 'text', 'calendar', 'time', 'time_masked', 'select', 'select2', 'radio', 'checkbox', 'textarea', 'images' ) );
} // End templatation_metabox_fieldtypes()
 ...
 * @return void
 */
function templatation_uploader_custom_fields( $pID, $id, $std, $desc ) {
...
return $uploader;
} // End templatation_uploader_custom_fields()
...
function woo_custom_enqueue ( $hook ) {
	wp_register_script( 'jquery-ui-datepicker', get_template_directory_uri() . '/functions/js/ui.datepicker.js', array( 'jquery-ui-core' ), false, true );
	wp_register_script( 'jquery-input-mask', get_template_directory_uri() . '/functions/js/jquery.maskedinput.js', array( 'jquery' ), '1.3', false, true );
	wp_register_script( 'woo-custom-fields', get_template_directory_uri() . '/functions/js/woo-custom-fields.js', array( 'jquery', 'jquery-ui-tabs' ), false, true );
		
  	if ( in_array( $hook, array( 'post.php', 'post-new.php', 'page-new.php', 'page.php' ) ) ) {
		wp_enqueue_script( 'jquery-ui-datepicker', false, true  );
		wp_enqueue_script( 'jquery-input-mask', false, true  );
  		wp_enqueue_script( 'woo-custom-fields', false, true  );
  	}
...
add_action( 'edit_post', 'templatation_metabox_handle', 10 );
add_action( 'admin_menu', 'templatation_metabox_add', 10 ); // Triggers templatation_metabox_create()
?>