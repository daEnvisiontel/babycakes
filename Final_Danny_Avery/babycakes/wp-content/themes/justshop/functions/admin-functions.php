		...
		foreach( $src_arr as $key => $custom_field ) {
			...
			if ( $link == 'img' ) {  // Just output the image
				...
				if ( ( is_single() || is_page() ) && $single == false ) {
					...
					if ( is_array($vt_image) && ( $vt_image['url'] || $is_auto_image ))  { // edited to fix notice @templatation JS v2.67
					/*if ( $vt_image['url'] || $is_auto_image ) {*/
						$src = wp_get_attachment_image_src( $thumb_id, 'full', true );
						$custom_field = esc_url( $src[0] );
					}
...
if ( ! function_exists( 'woo_twitter_script' ) ) {
	...
	<!--//--><![CDATA[//><!--
	    ...
	    function relative_time(time_value) {
	      ...
	      if (delta < 60) {
	        return '<?php esc_attr_e( 'less than a minute ago', 'templatation' ); ?>';
	      } else if(delta < 120) {
	        return '<?php esc_attr_e( 'about a minute ago', 'templatation' ); ?>';
	      } else if(delta < (60*60)) {
	        return (parseInt(delta / 60)).toString() + ' <?php esc_attr_e( 'minutes ago', 'templatation' ); ?>';
	      } else if(delta < (120*60)) {
	        return 'about an hour ago';
	      } else if(delta < (24*60*60)) {
	        return 'about ' + (parseInt(delta / 3600)).toString() + ' <?php esc_attr_e( 'hours ago', 'templatation' ); ?>';
	      } else if(delta < (48*60*60)) {
	        return '1 day ago';
	      } else {
	        return (parseInt(delta / 86400)).toString() + ' <?php esc_attr_e( 'days ago', 'templatation' ); ?>';
	      }
	    }
	//-->!]]>
	</script>
...
function woo_active_template( $filename = null ) {
	...
	trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s of the WooFramework! Please upgrade your Theme to the latest version.', 'templatation' ), 'woo_active_template', '5.4' ) );
...
function templatation_framework_update_page() {
	...
	delete_transient( 'wooframework_version_data' );
        ...
        $url = admin_url( 'admin.php?page=templatation_framework_update' );
        ?>
...
function templatation_framework_update_head() {
  if( isset( $_REQUEST['page'] ) ) {
	// Sanitize page being requested.
	$_page = esc_attr( $_REQUEST['page'] );

	if( $_page == 'templatation_framework_update' ) {
		//Setup Filesystem
		$method = get_filesystem_method();
		...
		if( $filesystem == false && $_POST['upgrade'] != 'Proceed' ) {
			...
			function templatation_framework_update_filesystem_warning() {
					$method = get_filesystem_method();
					echo "<div id='filesystem-warning' class='updated fade'><p>Failed: Filesystem preventing downloads. ( ". $method .")</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_filesystem_warning' );
				return;
		}
		...
		$temp_file_addr = download_url( esc_url( 'http://www.templatation.com/updates/framework.zip' ) );
		...
		if ( is_wp_error($temp_file_addr) ) {
			...
			//The source file was not found or is invalid
				function templatation_framework_update_missing_source_warning() {
					echo "<div id='source-warning' class='updated fade'><p>Failed: Invalid URL Provided</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_missing_source_warning' );
			} else {
				function templatation_framework_update_other_upload_warning() {
					echo "<div id='source-warning' class='updated fade'><p>Failed: Upload - $error</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_other_upload_warning' );
		...
		if ( is_wp_error($dounzip) ) {
			...
			if($error == 'incompatible_archive') {
				//The source file was not found or is invalid
				function templatation_framework_update_no_archive_warning() {
					echo "<div id='woo-no-archive-warning' class='updated fade'><p>Failed: Incompatible archive</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_no_archive_warning' );
			}
			if($error == 'empty_archive') {
				function templatation_framework_update_empty_archive_warning() {
					echo "<div id='woo-empty-archive-warning' class='updated fade'><p>Failed: Empty Archive</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_empty_archive_warning' );
			}
			if($error == 'mkdir_failed') {
				function templatation_framework_update_mkdir_warning() {
					echo "<div id='woo-mkdir-warning' class='updated fade'><p>Failed: mkdir Failure</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_mkdir_warning' );
			}
			if($error == 'copy_failed') {
				function templatation_framework_update_copy_fail_warning() {
					echo "<div id='woo-copy-fail-warning' class='updated fade'><p>Failed: Copy Failed</p></div>";
				}
				add_action( 'admin_notices', 'templatation_framework_update_copy_fail_warning' );
			}
...
// add_action( 'admin_head', 'templatation_framework_update_head' );
...
function woo_get_fw_version( $url = '', $check_if_critical = false ) {
	...
	if( ! empty( $url ) ) {
    	...
    	$fw_url = 'http://www.templatation.com/updates/functions-changelog.txt';
    }
...
$google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
						...
						array( 'name' => "Roboto", 'variant' => ':400,700,400italic,700italic')
...
if ( ! function_exists( 'woo_pagination' ) ) {
	function woo_pagination( $args = array(), $query = '' ) {
		...
		$defaults = array(
			...
			'current' => $current,
			'prev_next' => true,
			'prev_text' => __( '&larr; Previous', 'templatation' ), // Translate in WordPress. This is the default.
			'next_text' => __( 'Next &rarr;', 'templatation' ), // Translate in WordPress. This is the default.
			'show_all' => false,
			'end_size' => 1,
			'mid_size' => 1,
			'add_fragment' => '',
...
function woo_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&nbsp;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here :', 'templatation' ) . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'templatation' ),
		'echo' => true, 
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'templatation' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'templatation' ), get_the_time( __( 'i', 'templatation' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'templatation' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'templatation' ) ) . '">' . get_the_time( __( 'Y', 'templatation' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'templatation' ) ) . '">' . get_the_time( __( 'F', 'templatation' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'templatation' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'templatation' ) ) . '">' . get_the_time( __( 'Y', 'templatation' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'templatation' ), get_the_time( esc_attr__( 'W', 'templatation' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'templatation' ) ) . '">' . get_the_time( __( 'Y', 'templatation' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'templatation' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'templatation' ) );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'templatation' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'templatation' );
...
function woo_admin_bar_menu () {
	...
	$menu_label = __( 'templatation', 'templatation' );
	...
	// Main templatation Menu Item
	$wp_admin_bar->add_menu( array( 'id' => 'templatation', 'title' => $menu_label, 'href' => admin_url('admin.php?page=templatation') ) );

	// Theme Options
	$wp_admin_bar->add_menu( array( 'parent' => 'templatation', 'id' => 'templatation-theme-options', 'title' => __( 'Theme Options', 'templatation' ), 'href' => admin_url( 'admin.php?page=templatation' ) ) );

	if ( ( $super_user == $current_user_id ) || empty( $super_user ) ) {
		$wp_admin_bar->add_group( array( 'parent' => 'templatation', 'id' => 'templatation-super-user', 'meta' => array( 'class' => 'ab-sub-secondary' ) ) );

		// Framework Settings
		$wp_admin_bar->add_menu( array( 'parent' => 'templatation-super-user', 'id' => 'templatation-framework-settings', 'title' => __( 'Framework Settings', 'templatation' ), 'href' => admin_url( 'admin.php?page=templatation_framework_settings' ) ) );

		// Update Framework
/*		$wp_admin_bar->add_menu( array( 'parent' => 'templatation-super-user', 'id' => 'templatation-update-framework', 'title' => __( 'Update Framework', 'templatation' ), 'href' => admin_url( 'admin.php?page=templatation_framework_update' ) ) );
*/
		// Theme Version Data Display
		if ( true == $theme_data['is_child'] ) {
			$child_theme_name = sprintf( __( 'Child Theme: %s %s', 'templatation' ), $theme_data['child_theme_name'], $theme_data['child_theme_version'] );
			$wp_admin_bar->add_menu( array( 'parent' => 'templatation-super-user', 'id' => 'templatation-child-theme-version-data', 'title' => $child_theme_name ) );
		}

		$theme_name = sprintf( __( 'Theme: %s %s', 'templatation' ), $theme_data['theme_name'], $theme_data['theme_version'] );
		$wp_admin_bar->add_menu( array( 'parent' => 'templatation-super-user', 'id' => 'templatation-theme-version-data', 'title' => $theme_name ) );

		$framework_version = sprintf( __( 'WooFramework: %s', 'templatation' ), $theme_data['framework_version'] );
		$wp_admin_bar->add_menu( array( 'parent' => 'templatation-super-user', 'id' => 'templatation-framework-version-data', 'title' => $framework_version ) );
	} // End IF Statement
...
function wooframework_ajax_banner_close () {
	if( ! current_user_can( 'install_plugins' ) ) wp_die( __( 'You do not have sufficient permissions to access this page.', 'templatation' ) );
	
	if( ! check_admin_referer( 'wooframework_banner_close' ) ) wp_die( __( 'You have taken too long. Please go back and retry.', 'templatation' ) );
	...
	$sendback = esc_url(remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'ids' ), wp_get_referer() ));
...
/* WooSEO Deprecation Banner */
/*-----------------------------------------------------------------------------------*/
// removed @Templatation as we dont need it
/*-----------------------------------------------------------------------------------*/
/* T-i-mthumb Detection Banner */
/*-----------------------------------------------------------------------------------*/
// removed @Templatation as we dont need it
/*-----------------------------------------------------------------------------------*/
/* Static Front Page Detection Banner */
...
function wooframework_display_theme_version_data ( $echo = true ) {
	...
	// Framework Version
	$html .= '<span class="framework">' . esc_html( sprintf( __( 'Framework %s', 'templatation' ), $data['framework_version'] ) ) . '</span>' . "\n";
