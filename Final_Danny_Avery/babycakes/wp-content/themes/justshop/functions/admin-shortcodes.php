...
Optional arguments:
 ...
 - class: custom class
 - link: button link (e.g http://www.templatation.com)
 - window: true/false
...
function _dep_woo_shortcode_related_posts( $atts ) { trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s of the WooFramework! Please upgrade your WordPress to the latest version to use the latest functionality.', 'templatation' ), '_dep_woo_shortcode_related_posts', '5.4' ) ); }
...
function woo_shortcode_twitter($atts, $content = null) {
	...
	$output = '<div class="woo-sc-twitter ' . esc_attr( $float ) . '"><a href="' . esc_url( 'http://twitter.com/share' ) . '" class="twitter-share-button"'. $output .'>' . __( 'Tweet', 'templatation' ) . '</a><script type="text/javascript" src="' . esc_url ( 'http://platform.twitter.com/widgets.js' ) . '"></script></div>';
	return $output;
...
function woo_shortcode_toggle ( $atts, $content = null ) {
		...
		$defaults = array(
							'title_open' => __( 'Hide the Content', 'templatation' ),
							'title_closed' => __( 'Show the Content', 'templatation' ),
							'hide' => 'yes',
							...
							'read_more_text' => __( 'Read More', 'templatation' ),
							'read_less_text' => __( 'Read Less', 'templatation' )
						);
...
function woo_shortcode_contactform ( $atts, $content = null ) {
		$defaults = array(
						...
						'subject' => __( 'Message via the contact form', 'templatation' ),
						'button_text' => apply_filters( 'woo_contact_form_button_text', __( 'Submit', 'templatation' ) ),
		...
		if ( ( count( $_POST ) > 2 ) && isset( $_POST['submitted'] ) ) {
			...
			$error_responses = array(
									'contactName' => __( 'Please enter your name', 'templatation' ),
									'contactEmail' => __( 'Please enter your email address (and please make sure it\'s valid)', 'templatation' ),
									'contactMessage' => __( 'Please enter your message', 'templatation' )
									);
			...
			if ( count( $error_messages ) ) {} else {
				...
				// Send the e-mail.
				$headers = __( 'From: ', 'templatation') . $default_fields['contactName'] . ' <' . $default_fields['contactEmail'] . '>' . "\r\n" . __( 'Reply-To: ', 'templatation' ) . $default_fields['contactEmail'];
		...
		if( isset( $emailSent ) && $emailSent == true ) {
			$html .= do_shortcode( '[box type="tick"]' . __( 'Your email was successfully sent.', 'templatation' ) . '[/box]' );
			$html .= '<span class="has_sent hide"></span>' . "\n";
		}

		// Error messages
		if( count( $error_messages ) ) {
			$html .= do_shortcode( '[box type="alert"]' . __( 'There were one or more errors while submitting the form.', 'templatation' ) . '[/box]' );
		}

        // No e-mail address supplied.
        if( $email == '' ) {
			$html .= do_shortcode( '[box type="alert"]' . __( 'E-mail has not been setup properly. Please add your contact e-mail!', 'templatation' ) . '[/box]' );
		}
		...
		if ( $email == '' ) {} else {
			...
			if ( $show_default_fields != 'no' ) {
				...
				$html .= '<p><label for="contactName">' . __( 'Name', 'templatation' ) . '</label>' . "\n";
				...
				$html .= '<p><label for="contactEmail">' . __( 'Email', 'templatation' ) . '</label>' . "\n";
				...
				$html .= '<p class="textarea"><label for="contactMessage">' . __( 'Message', 'templatation' ) . '</label>' . "\n";
			...
			$html .= '<p class="inline"><input type="checkbox" name="sendCopy" id="sendCopy" value="true"' . esc_attr( $sendCopy ) . ' /><label for="sendCopy">' . __( 'Send a copy of this email to yourself', 'templatation' ) . '</label></p>' . "\n";
			...
			$html .= '<p class="screenReader"><label for="checking" class="screenReader">' . __('If you want to submit this form, do not enter anything in this field', 'templatation') . '</label><input type="text" name="checking" id="checking" class="screenReader" value="' . esc_attr( $checking ) . '" /></p>' . "\n";
...
function woo_shortcode_socialicon ( $atts, $content = null ) {
	...
	if ( $profile_type != '' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $_profile_to_display ); $_alt_text = sprintf( __( 'My %s Profile', 'templatation' ), $_alt_text ); } // End IF Statement
	...
	if ( $url == 'feed' ) {
		...
		if ( $title ) { $_alt_text = $title; } else { $_alt_text = __( 'Subscribe to our RSS feed', 'templatation' ); } // End IF Statement
		...
		foreach ( $_supported_profiles as $k => $v ) {
			...
			if ( $host == $v ) {
				...
				if ( $title ) { $_alt_text = $title; } else { $_alt_text = ucwords( $matches[1] ); $_alt_text = sprintf( __( 'My %s Profile', 'templatation' ), $_alt_text ); } // End IF Statement
...
function woo_shortcode_twitter_follow ( $atts, $content = null ) {
	...
	$output .= '<a href="'. esc_url( 'https://twitter.com/' . $username ) . '" class="twitter-follow-button"' . $tag_atts . '>' . __( 'Follow', 'templatation' ) . ' ' . $username . '</a>' . "\n";
