...
?>
<?php
// templatation Admin Interface

/*-----------------------------------------------------------------------------------

TABLE OF CONTENTS

- templatation Admin Interface - templatation_add_admin
- templatation Reset Function - woo_reset_options
- Framework options panel - templatation_options_page
- Framework Settings page - templatation_framework_settings_page
- woo_admin_head
- woo_load_only
- Ajax Save Action - woo_ajax_callback
- Generates The Options - templatation_machine
- templatation Uploader - templatation_uploader_function
- templatation Theme Version Checker - templatation_version_checker
- templatation Thumb Detection Notice - woo_thumb_admin_notice
- templatation Theme Update Admin Notice - woo_theme_update_notice
...
if ( ! function_exists( 'woo_ksesed_standalone_options' ) ) {
	function woo_ksesed_standalone_options() {
		return array( 'woo_footer_left_text', 'woo_footer_right_text', 'woo_connect_content', 'woo_ttext_text' );
	}
}
...
if ( ! function_exists( 'templatation_add_admin' ) ) {
	function templatation_add_admin() {
   	...
   	------------------------------------------------------------*/
  		...
  		------------------------------------------------------------*/
			...
			if ( $_action == 'reset' ) {
				...
				case 'templatation':
					...
					woo_reset_options( $options, 'templatation' );
					header( "Location: admin.php?page=templatation&reset=true" );
					die;

					break;

				case 'templatation_framework_settings':

					$options = get_option( 'woo_framework_template' );
					woo_reset_options( $options );
					header( "Location: admin.php?page=templatation_framework_settings&reset=true" );
					die;

					break;

				case 'templatation_seo':

					$options = get_option( 'woo_seo_template' );
					woo_reset_options( $options );
					header( "Location: admin.php?page=templatation_seo&reset=true" );
					die;

					break;

				case 'templatation_sbm':

					delete_option( 'sbm_woo_sbm_options' );
					header( "Location: admin.php?page=templatation_sbm&reset=true" );
					die;
		...
		if( function_exists( 'add_object_page' ) ) {
			add_object_page ( 'Page Title', __( 'Theme Options', 'templatation' ), 'manage_options', 'templatation', 'templatation_options_page', $icon );
		} else {
			add_menu_page ( 'Page Title', __( 'Theme Options', 'templatation' ), 'manage_options', 'templatation_home', 'templatation_options_page', $icon );
		}
		$woopage = add_submenu_page( 'templatation', $themename, __( 'Theme Options', 'templatation' ), 'manage_options', 'templatation', 'templatation_options_page' ); // Default

		// Framework Settings Menu Item
		$wooframeworksettings = '';
		if( $super_user == $current_user_id || empty( $super_user ) ) {
			$wooframeworksettings = add_submenu_page( 'templatation', __( 'Framework Settings', 'templatation' ), __( 'Framework Settings', 'templatation' ), 'manage_options', 'templatation_framework_settings', 'templatation_framework_settings_page' );
		}

		// templatation Content Builder
		if ( function_exists( 'templatation_content_builder_menu' ) ) {
			templatation_content_builder_menu();
		}

		// Update Framework Menu Item
/*		if( $super_user == $current_user_id || empty( $super_user ) ) {
			$woothemepage = add_submenu_page( 'templatation', 'WooFramework Update', 'Update Framework', 'manage_options', 'templatation_framework_update', 'templatation_framework_update_page' );
		}
*/
...
add_action( 'admin_menu', 'templatation_add_admin', 10 );
...
if ( ! function_exists( 'templatation_options_page' ) ) {
	function templatation_options_page() {
		global $pagenow;
...
?>
        <div id="header">
           <div class="logo">
                ...
                <?php } else { ?>
                <img alt="templatation" src="<?php echo esc_url( get_template_directory_uri() . '/functions/images/logo.png' ); ?>"/>
                <?php } ?>
            </div>
		...
		$return = apply_filters( 'woo_before_option_page', templatation_machine( $options ) );
?>
				...
				<div id="woo-nav">
					...
					<ul>
						<li class="top-level general current">
							<div class="arrow"><div></div></div><span class="icon"></span><a title="General Settings" href="#woo-option-error"><?php _e( 'Error', 'templatation' ); ?></a>
						</li>
					</ul>
				</div>
				<div id="content">
					<div class="group" id="woo-option-error" style="display: block; ">
						<div class="section section-info">
							<h3 class="heading"><?php _e( 'An Error Occured', 'templatation' ); ?></h3>
							<div class="option">
								<div class="controls">
									<p><?php _e( 'Something went wrong while trying to load your Theme Options panel.', 'templatation' ); ?></p>
									<p><?php echo sprintf( __( 'Please reload the page, if this error persists, please get in touch with us through our %1$s.', 'templatation' ), '<a href="' . esc_url( 'http://support.templatation.com' ) . '" target="_blank">' . __( 'Support Desk', 'templatation' ) . '</a>' ); ?></p>
								</div>
...
?>
			...
			jQuery( '#wooform' ).submit(function() {
					...
					var data = {
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'templatation' ) { ?>
						type: 'options',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'templatation_framework_settings' ) { ?>
						type: 'framework',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'templatation_seo' ) { ?>
						type: 'seo',
						<?php } ?>
						<?php if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'templatation_tumblog' ) { ?>
						type: 'tumblog',
						<?php } ?>
...
if ( ! function_exists( 'woo_load_only' ) ) {
	function woo_load_only() {
		...
		wp_enqueue_script( 'jquery-input-mask','','','',true );
		wp_enqueue_script( 'woo-scripts','','','',true );
		wp_enqueue_script( 'colourpicker','','','',true );
		wp_enqueue_script( 'woo-admin-interface','','','',true );
		wp_enqueue_script( 'woo-custom-fields','','','',true );
		wp_enqueue_script( 'jquery-ui-slider','','','',true );
		//@justshop1.0 modified above, making above loads on bottom.
		...
		wp_enqueue_script( 'woo-typography-preview','','','',true );
	} // End woo_load_only()
}
...
if ( ! function_exists( 'woo_framework_load_css' ) ) {
	function woo_framework_load_css () {
		...
		if (is_rtl()) {
		wp_register_style( 'woo-admin-interface-rtl', get_template_directory_uri() . '/functions/admin-style-rtl.css', '', '' );
		wp_enqueue_style( 'woo-admin-interface-rtl' );
		}
...
function woo_admin_message_success () {
	echo '<div class="updated fade" style="display: block !important;"><p>' . __( 'Options Saved Successfully', 'templatation' ) . '</p></div><!--/.updated fade-->' . "\n";
} // End woo_admin_message_success()

function woo_admin_message_error () {
	echo '<div class="error fade" style="display: block !important;"><p>' . __( 'There was an error while saving your options. Please try again.', 'templatation' ) . '</p></div><!--/.error fade-->' . "\n";
} // End woo_admin_message_error()

function woo_admin_message_reset () {
	echo '<div class="updated fade" style="display: block !important;"><p>' . __( 'Options Reset Successfully', 'templatation' ) . '</p></div><!--/.updated fade-->' . "\n";
...
if ( ! function_exists( 'templatation_machine' ) ) {
	function templatation_machine( $options ) {
		$counter = 0;
		$menu = '';
		$output = '';
		...
		foreach ( $options as $k => $value ) {
			...
			case "upload":
				$output .= templatation_medialibrary_uploader( $value['id'], $value['std'], null ); // New AJAX Uploader using Media Library
				break;
				
			case "upload_min":
				$output .= templatation_medialibrary_uploader( $value['id'], $value['std'], 'min' ); // New AJAX Uploader using Media Library
				break;
			...
			case "typography":
				...
				$output .= '<select class="woo-typography woo-typography-size woo-typography-size-px"  id="'. esc_attr( $value['id'].'_size_px') . '" '. $name_px . $show_px .'>';
					if( $val == 'default' ) { $active = 'selected="selected"'; } else { $active = ''; }
					$output .= '<option value="default" ' . $active . '>Default</option>'; 
				for ( $i = 9; $i < 71; $i++ ) {
			...
			case 'timestamp':
				...
				$output .= ' <span class="woo-timestamp-at">' . __( '@', 'templatation' ) . '</span> ';
			...
			if ( $value['type'] != "heading" && $value['type'] != "subheading" ) {
				...
				if ( !current_user_can( 'unfiltered_html' ) && isset( $value['id'] ) && in_array( $value['id'], woo_disabled_if_not_unfiltered_html_option_keys() ) )
					$explain_value .= '<br /><br /><b>' . esc_html( __( 'You are not able to update this option because you lack the <code>unfiltered_html</code> capability.', 'templatation' ) ) . '</b>';
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != 'templatation_content_builder' ) {
			$output .= '</div>';
		}
...
if ( ! function_exists( 'templatation_uploader_function' ) ) {
	function templatation_uploader_function( $id, $std, $mod ) {
		return templatation_medialibrary_uploader( $id, $std, $mod );
	} // End templatation_uploader_function()
}
