        ...
        public function panel_field_content( $field ) {
            ...
            switch( $field['type'] ) {
                ...
                case 'datepicker':
                    ...
                    $echo  = "<input type='text' id='{$id}_date' name='{$name}[date]' value='{$value['date']}' class='medium-text code panel-datepicker' colorpicker='" . __( 'Select a date', 'templatation' ) . "' /> - ";
                    $echo .= "<input type='text' id='{$id}_hh' name='{$name}[hh]' value='{$value['hh']}' class='small-text code' colorpicker='" . __( 'Hours', 'templatation' ) . "' /> : ";
                    $echo .= "<input type='text' id='{$id}_mm' name='{$name}[mm]' value='{$value['mm']}' class='small-text code' colorpicker='" . __( 'Minutes', 'templatation' ) . "' /> : ";
                    $echo .= "<input type='text' id='{$id}_ss' name='{$name}[ss]' value='{$value['ss']}' class='small-text code' colorpicker='" . __( 'Minutes', 'templatation' ) . "' />";
                    if( isset($field['description']) && $field['description'] != '' ) {
                        $echo .= "<p class='description'>{$field['description']}</p>";
                    }
                    break;
                ...
                case 'upload':
                    ...
                    $echo .= "  <input type='text' id='{$id}' name='{$name}' value='{$value}' class='regular-text code' /> <input type='button' name='' id='{$id}_button' class='button' value='". __('Upload', 'templatation') ."'>";
                    $echo .= '</div>';
                    if( isset($field['description']) && $field['description'] != '' ) {
                        $echo .= "<p class='description'>{$field['description']}</p>";
                    }
                ...
                case 'typography': $value = wp_parse_args( $value, $field['std'] ); ?>
                    <div class="typography_container typography">
                        <div class="option">
                            ...
                            <div class="select-wrapper font-unit">
                                <select class="typography_unit" name="<?php echo $name ?>[unit]" id="<?php echo $id ?>-unit">
                                    <option value="px" <?php selected( $value['unit'], 'px' ) ?>><?php _e( 'px', 'templatation' ) ?></option>
                                    <option value="em" <?php selected( $value['unit'], 'em' ) ?>><?php _e( 'em', 'templatation' ) ?></option>
                                    <option value="pt" <?php selected( $value['unit'], 'pt' ) ?>><?php _e( 'pt', 'templatation' ) ?></option>
                                    <option value="rem" <?php selected( $value['unit'], 'rem' ) ?>><?php _e( 'rem', 'templatation' ) ?></option>
                                </select>
                            </div>
                            ...
                            <div class="select-wrapper font-family">
                                <select class="typography_family" name="<?php echo $name ?>[family]" id="<?php echo $id ?>-family" data-instance="false">
                                    ...
                                    <?php else: ?>
                                        <option value=""><?php _e('Select a font family', 'templatation') ?></option>
                                    <?php endif ?>
                                </select>
                            </div>
                            ...
                            <div class="select-wrapper font-style">
                                <select class="typography_style" name="<?php echo $name ?>[style]" id="<?php echo $id ?>-style">
                                    <option value="regular" <?php selected( $value['style'], 'regular' ) ?>><?php _e( 'Regular', 'templatation' ) ?></option>
                                    <option value="bold" <?php selected( $value['style'], 'bold' ) ?>><?php _e( 'Bold', 'templatation' ) ?></option>
                                    <option value="extra-bold" <?php selected( $value['style'], 'extra-bold' ) ?>><?php _e( 'Extra bold', 'templatation' ) ?></option>
                                    <option value="italic" <?php selected( $value['style'], 'italic' ) ?>><?php _e( 'Italic', 'templatation' ) ?></option>
                                    <option value="bold-italic" <?php selected( $value['style'], 'bold-italic' ) ?>><?php _e( 'Italic bold', 'templatation' ) ?></option>
                                </select>
                            </div>
                        ...
                        <div class="font-preview">
                            ...
                            <!-- Refresh -->
                            <div class="refresh_container"><button class="refresh"><?php _e( 'Click to preview', 'templatation' ) ?></button></div>
                        </div>
                    </div>
                    <?php
                        global $yith_panel_if_typography;
