...
class Woo_Featured_Products extends WP_Widget {
	...
	function Woo_Featured_Products() {
		$widget_ops = array( 'description' => 'Display featured products (use in the homepage widget region only, will not showup in sidebars)' );
		parent::WP_Widget( false, __( 'JS - Featured Products Loop', 'templatation' ), $widget_ops );
	}
		...
		extract( $instance, EXTR_SKIP );
			...
			echo $before_widget;
				...
				if ( is_woocommerce_activated() ) {
					...
					echo '<div class="woocommerce woocommerce-columns-4">';
					...
					while ( $loop->have_posts() ) : $loop->the_post(); $product; ?>
						...
						<li class="product featured  <?php if( defined( 'YITH_WCWL' ) ) echo " wcwl "; if ( $i % 4 == 0 ) { echo 'last'; } ?>">
							...
							<?php
								echo '<div class="excerpt">' . justshop_truncate( get_the_excerpt(), 11 ) . '</div>';
	...
	function woo_enforce_defaults( $instance ) {
		...
		if ( '' == $instance['products_per_page'] ) {
			$instance['products_per_page'] = __( '4', 'templatation' );
		}
		return $instance;
	}
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('products_per_page'); ?>"><?php _e('Number of featured products to display (Multiples of 4 recommended):','templatation'); ?></label>
