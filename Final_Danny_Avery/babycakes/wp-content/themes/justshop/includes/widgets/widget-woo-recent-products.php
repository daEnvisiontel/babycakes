...
class Woo_Recent_Products extends WP_Widget {
	...
	function Woo_Recent_Products() {
		$widget_ops = array( 'description' => 'Display recent products (use in the homepage widget region only, will not showup in sidebars)' );
		parent::WP_Widget( false, __( 'JS - Recent Products Loop', 'templatation' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		extract( $instance, EXTR_SKIP );
			...
			echo $before_widget;
	    		...
	    		echo '<h1>' . __('New Products', 'templatation') . '</h1>';
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('products_per_page'); ?>"><?php _e('Number of recent products to display (Multiples of 4 recommended):','templatation'); ?></label>
