...
class Woo_Best_Selling_Products extends WP_Widget {
	...
	function Woo_Best_Selling_Products() {
		$widget_ops = array( 'description' => 'Display best selling products (use in the homepage widget region only, will not showup in sidebars)' );
		parent::WP_Widget( false, __( 'JS - Best Selling Products Loop', 'templatation' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		extract( $instance, EXTR_SKIP );
			...
			echo $before_widget;
				...
				echo '<h1>' . __( 'Best Sellers', 'templatation' ) . '</h1>';
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('products_per_page'); ?>"><?php _e('Number of best selling products to display (multiples of 4 recommended):','templatation'); ?></label>
