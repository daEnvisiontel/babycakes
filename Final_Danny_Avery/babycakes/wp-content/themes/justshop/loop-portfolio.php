...
$settings = array(
                    ...
                    'portfolio_title' => __('Portfolio', 'templatation')					
				);
...
if ( $wp_query->have_posts() ) : $count = 0; ?>
	<header class="<?php echo $filtering_css_class; ?> section-title portfolio" data-option-key="filter">

      <?php if ( isset( $settings['portfolio_title'] ) && ( $settings['portfolio_title'] != '' ) ) { ?>
            <h1 class="page-title"><?php echo esc_attr( $settings['portfolio_title'] ); ?></h1>
        <?php } ?>
...
?>
			...
			$post_classes[] = 'portfolio'; //@4.6 
			?>
			<article id="portfolio-item-id-<?php the_ID(); ?>" <?php post_class( $post_classes ); ?> data-category="<?php echo $post_tags[0] ;?>" >
			...
			<div class="content-wrapper <?php echo $port_type; ?>">
				...
				<div class="portfolio-title overflow">
					...
					<p class="scheme-b"><?php echo $commaList = implode(', ', $post_tags); ?> </p>
				</div>

						<div class="fix"></div>
    					<?php global $more; $more = 0;?>	                                        
                        <?php the_excerpt(); ?>
						<a class="button alt" href="<?php the_permalink(); ?>"><?php _e('View Details', 'templatation') ?></a> <?php if ($custom_url != ''): ?><a class="button" href="<?php echo $custom_url ?>"><?php _e('Visit Project', 'templatation') ?></a><?php endif; ?>
		
			</div>

			<?php } else { // if not one column portfolio ?>
			<div class="content-wrapper not-one <?php echo $port_type; ?>">

				<figure class="over">
				<a <?php echo $settings['rel']; ?> title="<?php echo $settings['caption']; ?>" href="<?php echo $settings['large']; ?>" class="item">
					<?php echo $image; ?>
					</a>
				</figure>
