...
if ( post_password_required() ) { ?>
	<p class="nocomments"><?php _e( 'This post is password protected. Enter the password to view comments.', 'templatation' ); ?></p>
<?php return; } ?>
...
<div id="comments">
	...
	<?php if ( ! empty( $comments_by_type['comment'] ) ) { ?>
		<h2><span><?php comments_number( __( 'No Responses', 'templatation' ), __( 'One Response', 'templatation' ), __( '% Responses', 'templatation' ) ); ?> <?php _e( 'to', 'templatation' ); ?> &#8220;<?php the_title(); ?>&#8221;</span></h2>
	...
	<?php if ( ! empty( $comments_by_type['pings'] ) ) { ?>
        ...
        <h2 id="pings"><?php _e( 'Trackbacks/Pingbacks', 'templatation' ); ?></h2>
...
<?php } else { // this is displayed if there are no comments so far ?>
	...
	<?php
		...
		if ( comments_open() && is_singular() ) { ?>
			<div id="comments">
				<h5 class="nocomments"><?php _e( 'No comments yet.', 'templatation' ); ?></h5>
			</div>
		<?php } ?>
